...
def createDirectories():
    try:
        os.mkdir(os.getcwd() + "/datasetv3")
        os.mkdir(os.getcwd() + "/datasetv3/training_set")
        os.mkdir(os.getcwd() + "/datasetv3/test_set")
        for i in range(1, 33):
            os.mkdir(os.getcwd() + "/datasetv3/training_set/" + str(i) + "/")
            os.mkdir(os.getcwd() + "/datasetv3/test_set/" + str(i) + "/")
    except:
...
def generateImages(num_images, path):
    for crt in range(num_images):
        ...
        plt.tight_layout()
        plt.axis('off')
        fig.savefig(os.getcwd() + "/datasetv3/"+ path + "/" + str(totalImg) + "/" + str(crt) + '.png', bbox_inches="tight", dpi=100)


        #Save auxiliary info to folder
        """
        f = open(os.getcwd() + "/datasetv3/"+ path +"/" + str(totalImg) + "/" + str(crt) + '.csv', 'w')
        ...
        # Save the image
        fig.savefig(os.getcwd() + "/datasetv3/"+ path +"/" + str(totalImg) + "/" + str(crt) + '_res.png', bbox_inches="tight", dpi=100)
        plt.close(fig)
        print("Generated to: ", os.getcwd() + "/datasetv3/"+ path +"/" + str(totalImg) + "/" + str(crt) + '_res.png')
        """
